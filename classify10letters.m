...
delimiterIn = ' ';
headerlinesIn = 1;
data_training = importdata('B1_training_withoutx.txt',delimiterIn,headerlinesIn);
...
delimiterIn = '&';
data_testing = importdata('B1_testing_withoutx.txt',delimiterIn,headerlinesIn);
...
for i=1:length(data_training)
    [ignore, BMU(i)]=closest(data_training(i,:),W_training);
    BMU_training(i,:)=Grid(BMU(i),:);
    plot(BMU_training(i,1),BMU_training(i,2),'');
    text(BMU_training(i,1),BMU_training(i,2),labels_training{i},'FontSize',15,'Color','black');
    xlim([1 s]);
    ylim([1 s]);
    hold on;
    grid on;
...
xlim([1 s]);
ylim([1 s]);
title(sprintf('Classification of test set\n with niter=%d, D_0=%d, L_0=%.2f, lambda_D=%.2f, lambda_L=%.2f',niter,D0,L0,lambda_D,lambda_L));
grid on;
